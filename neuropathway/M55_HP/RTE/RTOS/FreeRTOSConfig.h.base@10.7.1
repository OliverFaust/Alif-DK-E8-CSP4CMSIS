...
//  <i> Default: 8192
/* 256KB: Inference's std::vector<uint8_t*>/std::vector<Span<uint8_t>>
 * (inference_process.cpp, sized to method_meta.num_memory_planned_
 * buffers()) plus whatever ExecuTorch's own MemoryAllocator/
 * HierarchicalAllocator machinery allocates internally via operator new
 * (routed through csp4cmsis's glue.cpp) are all transient, per-window
 * heap traffic -- not yet measured against a real high-water mark on this
 * specific model, so kept at the same generous figure the prior
 * (native-template) config used rather than re-derived from scratch.
 * MAIN_APP_STACK_WORDS (csp/csp4cmsis_spn.cpp) and each CSProcessStatic<N>
 * stack are separate, statically-allocated FreeRTOS task stacks -- they
 * don't draw from this heap at all. */
...
//  <i> Default: 0
/* main.cpp defines vApplicationMallocFailedHook() -- catch heap
 * exhaustion (this project's heap backs real ExecuTorch/inference
 * allocations, not just incidental C++ new) instead of silently
 * returning NULL from pvPortMalloc(). */
...
//  <i> Default: 1
/* main.cpp provides its own vApplicationGetIdleTaskMemory/
 * vApplicationGetTimerTaskMemory, so the kernel must not also define them
 * (duplicate-symbol link error otherwise). */
...
//  <i> This setting is only relevant on ARMv8-M ports.
//  <i> Default: 0
/* RTSS-HP is a Cortex-M55: FPU and MVE are both present, and this
 * project's ExecuTorch/Ethos-U inference path actively uses MVE. */
...
/* main.cpp's vApplicationGetIdleTaskMemory()/vApplicationGetTimerTaskMemory()
 * signatures use configSTACK_DEPTH_TYPE for their stack-size out-param;
 * this template doesn't define it (falls back to tasks.h's own default of
 * uint16_t), so pin it to size_t explicitly to match those definitions
 * exactly. */
