...
//  <i> Default: 8192
/* csp4cmsis needs dynamic allocation too (unlike blinky's all-static task):
 * application.cpp's MainApp_Task is a plain xTaskCreate() with a 4096-word
 * (16KB) stack; each csp::Channel (sync_channel.cpp) heap-allocates a mutex
 * + two semaphores via osMutexNew()/osSemaphoreNew(); Barrier (barrier.cpp,
 * compiled in but unused by this smoke test) does the same; and glue.cpp
 * routes global `operator new`/`delete` through pvPortMalloc()/vPortFree(),
 * so any incidental heap C++ allocation lands here too. 8KB (blinky's
 * budget, and this template's own default) was nowhere near enough on its
 * own for just the MainApp stack; sized generously since SRAM0/1 have 4MB
 * each free and DTCM has ~950KB free after everything else in the Debug
 * build. */
...
//  <i> Default: 0
/* main.cpp defines vApplicationMallocFailedHook() -- catch heap exhaustion
 * now that the heap backs real dynamic allocations (MainApp's stack,
 * per-channel mutex/semaphores, operator new via glue.cpp) instead of
 * silently returning NULL from pvPortMalloc(). */
...
//  <i> This setting is only relevant on ARMv8-M ports.
//  <i> Default: 0
/* RTSS-HP is a Cortex-M55: FPU and MVE are both present. */
